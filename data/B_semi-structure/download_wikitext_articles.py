"""
从 EleutherAI/wikitext_document_level 下载 wikitext-103-raw-v1 的 train split，
将每一篇文章保存为一个本地 .txt 文件。

策略：
- 只保留长度在 [MIN_CHARS, MAX_CHARS] 区间内的文章（过短/过长都跳过）
- 对真正保存的文章，用字符数近似 token 数进行累积
- 当累计 token 数 >= TARGET_TOKENS 时停止

配置：
- 输出目录：D:\\code\\Github\\SLAC-test\\data\\B_semi-structure\\en-tech
- 日志文件：D:\\code\\Github\\SLAC-test\\log\\wiki-en.log
"""
...
OUTPUT_DIR = Path(r"D:\code\Github\SLAC-test\data\B_semi-structure\en-tech")
LOG_FILE = Path(r"D:\code\Github\SLAC-test\log\wiki-en.log")
...
# 单篇字符数区间
MIN_CHARS = 10_000   # 至少 1 万字符
MAX_CHARS = 20_000   # 不超过 2 万字符

# 目标“token”总数（这里用字符数近似 token 数）
TARGET_TOKENS = 8_000_000

# 最多扫描多少篇（不是保存数，仅是安全上限；None 表示整个 split）
MAX_SAMPLES_TO_SCAN = None
...
def main():
    ...
    for idx, ex in tqdm(
        enumerate(iterator),
        total=len(iterator),
        desc=f"Saving en wiki articles ({SPLIT})",
    ):
        total_scanned += 1
        ...
        try:
            ...
            saved_docs += 1
            total_tokens += length  # 这里用字符数近似 token 数
