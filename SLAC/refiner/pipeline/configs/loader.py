...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument(
        ...
        nargs="+",
        default=None,
        help="Input files or directories. Supported: pdf, docx, txt, text, md, json. "
             "Required unless --reuse_atoms_b0_jsonl is provided.",
    )
    p.add_argument(
        "--output_dir",
        required=True,
    ...
    p.add_argument(
        "--recursive",
        action="store_true",
        help="Recursively scan directories for supported files in stage 1.",
    )
    p.add_argument(
        "--domain",
        default=None,
    ...
    # PDF reader config
    # -----------------------------
    p.add_argument("--pdf_max_pages", type=int, default=0)
    p.add_argument("--pdf_min_font_size", type=float, default=5.0)
    p.add_argument("--pdf_max_weird_ratio_span", type=float, default=0.35)
    p.add_argument("--disable_pdf_drop_tiny_spans", action="store_true")
    p.add_argument("--pdf_garbled_page_weird_ratio", type=float, default=0.25)
    p.add_argument("--pdf_garbled_min_len", type=int, default=10)
    p.add_argument("--disable_pdf_ocr_on_images", action="store_true")
    p.add_argument("--pdf_ocr_always_on_images", action="store_true")
    p.add_argument("--pdf_ocr_dpi", type=int, default=300)
    p.add_argument("--pdf_ocr_lang", default="chi_sim+eng")
    p.add_argument("--pdf_ocr_backend", default="tesseract")
    p.add_argument("--disable_pdf_enable_pdftotext", action="store_true")
    p.add_argument("--pdf_ocr_when_text_short", type=int, default=120)
    p.add_argument("--pdf_image_area_ratio_for_ocr", type=float, default=0.35)
    ...
    p.add_argument(
        "--infer_script",
        default=None,
        help="Optional override for SLAC/refiner/scripts/infer_bestofn.py path.",
    )
...
def build_stage1_cmd(args: argparse.Namespace, stage1_output_dir: Path) -> List[str]:
    ...
    if args.disable_pdf_drop_tiny_spans:
    ...
    if args.disable_pdf_ocr_on_images:
    ...
    if args.disable_pdf_enable_pdftotext:
...
def build_stage2_cmd(args: argparse.Namespace, atoms_b0_jsonl: Path, stage2_output_dir: Path) -> List[str]:
    ...
    if args.disable_export_doc_catalog:
