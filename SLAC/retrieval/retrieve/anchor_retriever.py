...
class AnchorRetriever:
    ...
    def search(
        ...
        for qv in [q for q in query_variants if str(q).strip()]:
            ...
            for h in hits:
                ...
                if prev is None or h.score > prev.score or (h.exact_match and not prev.exact_match):
    ...
    def _search_one(self, query: str, topk: int = 30) -> List[AnchorHit]:
        ...
        for object_id, meta in self.object_meta.items():
            ...
            if number_sig and number_sig in q_norm:
                exact_boost_ids.add(object_id)
            elif number_sig and q_norm in number_sig:
                exact_boost_ids.add(object_id)
            elif path_text and q_norm and q_norm in path_text:
                exact_boost_ids.add(object_id)
            elif anchor_text and q_norm and q_norm in anchor_text:
                exact_boost_ids.add(object_id)
